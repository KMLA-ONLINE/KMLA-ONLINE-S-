...
- 댓글 수: `create_post_comment`와 `delete_post_comment` 모두 트리거가 갱신한 게시물의
  `comment_count`를 돌려준다. 상세는 그 값을 그대로 쓰고 손에 든 값에 `±1` 하지 않는다. 생성
  쪽은 그 사이 다른 사람이 단 댓글을 조용히 지운 수가 되기 때문이고, 삭제 쪽은 지운 댓글
  하나로 끝나지 않기 때문이다 — 최상위는 답글 묶음을 통째로 데려가고 자식이 없어진 자리
  표시는 조상까지 이어서 사라지므로(기능 명세 §9.4), 클라이언트가 빼려면 그 규칙을 한 벌 더
  적어야 한다.
  ...
  `max`로 거를 수 없다. 오가는 값이 모두 서버가 센 절대값이라 같은 값을 두 번 얹어도 결과가
  같고, 늦게 온 예전 응답이 최신 수를 덮는 일은 두 RPC가 작업 전에 게시물 행을 `for update`로
  잠그는 것으로 막는다. 한 게시물의 댓글 뮤테이션은 DB에서 직렬화되므로 나중 요청은 앞 요청이
  커밋된 뒤의 수를 읽는다. 클라이언트가 뮤테이션을 겹쳐 보내도 이 순서는 성립한다. 이 경로는
