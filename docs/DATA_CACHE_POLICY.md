...
- 보호된 조회 결과와 signed URL을 `localStorage`, IndexedDB 또는 Service Worker Cache Storage에
  ...
  - 저장소의 주인이 바뀌면 `localStorage` 계정 키와 **같은 시점에** 캐시를 통째로 지운다.
    ...
    함께 덮는다. `QueryProvider`는 삭제가 성공하기 전 새 사용자의 화면을 그리지 않고, 실패하면
    새 owner를 기록하지 않은 채 재시도한다. 남는 구멍은 이미 날아간 이전 이미지 요청 하나가
    삭제 뒤에 도착해 캐시를 다시 채우는 경우뿐이다. 페이지에서 Service Worker의 쓰기를 취소할
    방법이 없어 받아들인 위험이다.
  - 캐시 이름은 앱과 빌드 스크립트 두 곳에 적혀 있고, 어긋나면 계정을 바꿔도 이전 사용자의
    이미지가 남는데 그것이 조용히 일어난다. `scripts/build-sw.mjs`가 빌드 때 두 리터럴이
    같은지 확인하고 다르면 빌드를 세운다.
  - 캐시 키를 다시 쓰면 `ExpirationPlugin`을 함께 쓸 수 없다. 그 플러그인은 타임스탬프를
    ...
    응답에 저장 시각 헤더를 붙이고 cache hit 전에 24시간을 검사해 직접 지운다. 따라서 만료한
    signed URL이 있더라도 Cache Storage가 Storage/RLS 검사를 우회해 본문을 24시간보다 오래
    돌려주지 않는다.
    상한도 같은 키로 직접 건다(`cacheDidUpdate`에서 `cache.keys()` 순서대로 잘라낸다).
  ...
  - 목록 축소본을 우선 보관하기 위해 본문이 1MiB를 넘는 응답은 캐시하지 않는다. 최대 300개라
    해도 원본 이미지가 Cache Storage를 수 GiB까지 키우지 않게 하는 상한이다.
...
- 수동 새로고침은 현재 화면 재검증 전에 관련 캐시와 셸 알림 뱃지를 stale 상태로 만든다. 캐시가
  새로고침 요청을 가로막아서는 안 된다. 자식 화면의 명시적 재검증은 인증 게이트의 프로필을 다시
  읽지 않는다.
...
| 셸 알림 뱃지               |       1분 | 아래 설명 참고. 게이트 로더는 이 값을 무시하고 항상 새로 읽는다.     |
...
그룹 홈도 2분을 그대로 쓰고 새 게시물 수만 따로 챙긴다(기능 명세 §7.19). 그룹 게시물 목록이 실제로
표시되면 그 자리에서 그룹 홈 캐시의 해당 그룹 `new_post_count`를 0으로 낮추고, 이어서
`mark_group_posts_visited`로 멤버십의 `posts_visited_at`을 서버 시각으로 갱신한 뒤 홈을 무효화한다.

낮추는 일을 RPC 응답보다 먼저 하는 이유는 방문 저장이 끝나기 전에 목록으로 돌아오는 경로 때문이다.
그룹 홈에는 화면 observer가 없다 — `GroupHomeScreen`은 `useQuery` 없이 로더 snapshot을 그대로
받으므로, 무효화는 다음 로더 실행에만 영향을 주고 이미 그려진 화면을 다시 읽히지 못한다. 그래서
`staleTime: 0`으로 진입마다 네 요청을 전부 다시 받는 대신, 내 기기에서 일어난 변화는 캐시를 직접
고쳐 반영하고 무효화는 다음 진입을 위해 남긴다. 상세 직접 진입이나 다른 그룹 탭은 방문을 기록하지 않는다.
...
단, 게이트 로더만은 `staleTime`을 0으로 덮어 항상 새로 읽는다. 게이트가 새로 마운트되는
순간이 곧 뱃지가 틀렸을 수 있는 순간이기 때문이다. Push 알림 경로가 그렇다 —
`resolve_my_notification_destination()`이 `read_at`을 찍는데 그 라우트는 게이트 밖에 있어,
목적지로 넘어오며 게이트가 다시 마운트된다. 캐시된 값을 쓰면 방금 읽은 알림이 1분 동안 안 읽음으로
남는다. 같은 이유로 게이트 밖을 지나는 동안 놓친 Realtime 이벤트도 여기서 복구된다.
...
- 댓글과 반응: `['posts', 'engagement', postId]`는 네트워크 query가 아닌 표시용 TanStack Query
  overlay다. 카드·행·상세는 서버 snapshot 위에 이 partial 값을 얹고, 목록 원본과 미디어 수화본은
  고치지 않는다. 댓글 생성·삭제는 RPC가 반환한 절대 `comment_count`만 기록한다. 반응은
  `reaction_count`와 `my_reaction`만 optimistic으로 기록하고 `top_reactions`는 서버 정본으로만
  갱신하며, 실패하면 반응 세 필드만 직전 값으로 되돌린다. 그래서 늦은 수화본도 최신 표시를
  되돌리지 않고 같은 게시물을 그리는 모든 화면이 함께 갱신된다. 이 경로는 랭킹과 무관하므로
  세션을 새로 열지 않는다. account 전환의 `queryClient.clear()`가 overlay도 제거한다.
- engagement overlay를 제거하는 건 당겨서 새로고침 하나뿐이다. 화면 전체를 다시 읽는 새로고침만
  새 서버 snapshot을 기준으로 삼을 수 있기 때문이다. 그룹 가입이나 글 저장이 여는 새 세션은
  피드만 다시 읽으므로 overlay를 건드리지 않는다 — 비우면 다른 화면에 열려 있는 게시물이 방금
  성공한 반응·댓글 수를 로더 snapshot으로 되돌린 채 남는다. 상세를 닫거나 카테고리를 전환할
  때도 제거하지 않는다.
- 댓글 뒤 route 재검증은 하지 않는다. 재검증하면 사용자가 펼쳐 둔 답글 묶음과 위로 불러온 이전
  페이지가 초기화된다. 대신 피드의 상세는 같은 게시물을 다시 열 때 명시적으로 다시 읽어, 닫혀
  있는 동안 남아 있던 `useFetcher` 응답이 방금 쓴 댓글을 빠뜨리지 않게 한다.
...
- 다른 관리자가 권한을 회수하는 경우에는 현재 열린 그룹 화면을 창 focus 복귀 시 다시 읽는다.
  먼저 `['groups']`를 stale 처리해 2분 fresh cache를 건너뛰고, RLS 결과가 기존 loader snapshot을
  덮는다. 다른 route의 focus 복귀는 이 목적으로 재검증하지 않는다.
...
- 이전 페이지는 `(last_activity_at, id)` 커서로 현재 알림함 화면에만 병합하고 화면을 벗어나면 폐기한다.
  이전 페이지 resource loader는 전역 재검증에 참여하지 않고 사용자가 더 보기를 누를 때만 실행한다.
- 셸의 최근 24시간 badge는 라우트 로더가 아니라 `["notifications", "badge"]` 쿼리가 소유한다.
  알림 Realtime 변경과 창 focus 복귀는 **이 키만 무효화한다.** 개별·전체 읽음 POST 뒤에는
  게이트 loader가 이 쿼리를 항상 새로 읽으므로 action에서 다시 무효화하지 않는다.
- 알림함 목록은 로더가 소유하므로, 위 세 신호는 사용자가 실제로 `/noti`에 있을 때만 라우트를
  함께 재검증한다. 같은 URL의 명시적 재검증은 자식 로더만 다시 실행하고 인증 게이트의 프로필은
  다시 읽지 않는다. 짧은 구간에 몰린 Realtime 이벤트는 첫 페이지 재검증 한 번으로 합친다.
- badge를 게이트 로더에 두지 않는다. badge 하나를 갱신하려고 `revalidate()`를 부르면 지금 보고
  있는 라우트의 로더까지 함께 돈다.
...
로그인하는 경로가 모두 덮인다. Cache Storage 삭제가 끝나기 전에는 새 사용자 UI를 렌더하지 않고,
실패하면 owner key를 보존해 같은 계정에도 삭제를 다시 시도한다. 기능 코드가 직접 지우지 않는다.
