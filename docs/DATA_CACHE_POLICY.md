...
그룹 홈은 예외로 `/groups` 로더에서 `staleTime: 0`으로 덮어 진입·새로고침마다 새 게시물 수를
함께 읽는다(기능 명세 §7.19). 화면 observer는 2분 정책을 유지해 로더 직후 중복 조회를 막는다.
그룹 게시물 목록이 실제로 표시되면 `mark_group_posts_visited`로 멤버십의 `posts_visited_at`을
서버 시각으로 갱신하고 그룹 홈 캐시를 무효화한다. 방문 저장 도중 목록으로 돌아온 경우에도
활성 observer가 다시 조회하여 배지를 갱신한다. 상세 직접 진입이나 다른 그룹 탭은 방문을 기록하지 않는다.
