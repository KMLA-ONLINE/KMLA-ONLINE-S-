...
- 댓글 수: `create_post_comment`는 insert 트리거가 갱신한 게시물의 `comment_count`를 댓글 행과
  함께 돌려준다. 상세는 그 값을 그대로 쓰고, 손에 든 값에 `+1` 하지 않는다 — 그 사이 다른
  사람이 단 댓글을 조용히 지운 수가 되기 때문이다. 삭제 RPC에는 정본 수가 없어 그쪽만 상대
  계산으로 남는다.
  상세는 생성과 삭제 양쪽에서 이 수를 연 목록에 알리고, 목록은 `patchFeedPostCommentCount`와
  `patchGroupPostCommentCount`로 자기 캐시의 해당 게시물만 고친다. 한쪽만 알리면 목록이
  "댓글은 늘지만 줄지는 않는" 상태로 어긋난다. 그룹 게시물은 피드와 그룹 목록 양쪽에 들어
  있으므로 둘 다 고치고, 그룹 목록은 `(categoryId, cursor)` 엔트리가 여러 개라 해당 그룹의
  모든 페이지 엔트리를 훑는다. 받은 값을 그대로 덮는다 — 삭제로 줄어든 수도 반영되어야 해서
  `max`로 거를 수 없다. 늦게 도착한 예전 응답이 최신 수를 낮추는 것은, 한 게시물의 댓글
  뮤테이션이 상세의 `pending`으로 직렬화되어 애초에 겹치지 않는 것으로 막는다. 이 경로는
  랭킹과 무관하므로 세션을 새로 열지 않는다.
