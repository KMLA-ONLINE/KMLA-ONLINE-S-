...
- 보호된 조회 결과와 signed URL을 `localStorage`, IndexedDB 또는 Service Worker Cache Storage에
  ...
  - 저장소의 주인이 바뀌면 `localStorage` 계정 키와 **같은 시점에** 캐시를 통째로 지운다.
    ...
    함께 덮는다. 남는 구멍이 하나 있다: 지우는 시점에 이미 날아가 있던 이미지 요청은 그
    뒤에 도착해 캐시를 다시 채울 수 있다. 창은 요청 하나 길이이고, 그 이미지를 보려면 기기의
    Cache Storage를 직접 열어야 한다. 페이지에서 Service Worker의 쓰기를 취소할 방법이 없어
    받아들인 위험이다.
