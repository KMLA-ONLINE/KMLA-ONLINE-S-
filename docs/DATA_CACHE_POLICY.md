...
- 댓글 생성의 정본 댓글 수: `create_post_comment`는 insert 트리거가 갱신한 게시물의
  `comment_count`를 댓글 행과 함께 돌려준다. 상세는 그 값을 그대로 쓰고, 손에 든 값에 `+1`
  하지 않는다 — 그 사이 다른 사람이 단 댓글을 조용히 지운 수가 되기 때문이다.
  상세는 이 수를 연 목록에 알리고, 목록은 `patchFeedPostCommentCount`와
  `patchGroupPostCommentCount`로 자기 캐시의 해당 게시물만 고친다. 그룹 게시물은 피드와 그룹
  목록 양쪽에 들어 있으므로 둘 다 고치고, 그룹 목록은 `(categoryId, cursor)` 엔트리가 여러 개라
  해당 그룹의 모든 페이지 엔트리를 훑는다. 값은 `max`로 덮어 늦게 도착한 예전 응답이 최신 수를
  낮추지 못하게 한다. 이 경로는 랭킹과 무관하므로 세션을 새로 열지 않는다.
