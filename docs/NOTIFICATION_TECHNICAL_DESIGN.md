...
익명 활동 제한, 메시지, 사용자 지정 그룹 초대, 노래방 참여자, 예약 리마인더,
...
`private.notification_event_keys`가 원본 활동의 안정적인 event key를 보관한다. 멘션은
`post-mention:<post>:<profile>` 형태의 key를 써서, 수정 때마다 멘션 행을 지우고 다시 넣어도
이미 알린 대상은 조용하고 처음 등장한 대상만 새 알림을 받는다. 반응은 행위자별 event
...
삭제는 하드 삭제다. `delete_group`이 그 그룹을 가리키는 기존 알림을 먼저 지운 뒤 그룹 행을 지우고,
그룹 삭제 알림은 `group_id` 없이 그룹 이름을 제목에 실어 보낸다(삭제 및 보존 정책 §5.2). 따라서
이름이 비어서 나오는 알림은 그룹 삭제 알림과 애초에 그룹과 무관한 알림뿐이다.

같은 함수는 댓글·답글 알림에 `comment_excerpt`(댓글 본문 앞 200자)를, 반응 알림에 `reaction`(표시된
반응자의 현재 반응 종류)을 함께 반환한다. 두 값 모두 알림 행에 복사하지 않고 조회 시점에 조인하므로
댓글 수정·삭제와 반응 변경·제거가 그대로 반영된다. 본문은 `private.can_read_post`가 참이고 댓글이
삭제 표시되지 않았을 때만 싣는다. Push 제목과 본문은 여전히 `title`과 kind별 고정 문구만 쓴다.
...
아직 현재 entry인 출발 화면을 덮어쓴다. 로그인과 알림함으로 빠지는 갈래는 loader가 즉시 replace하며 로그인 화면도 같은 이유로
남기지 않는다.

밑에 무엇을 깔지는 **돌아갈 내역이 있는가** 하나로 갈린다. 앱이 이미 떠 있던 창이면 그것으로
끝이며, 알림함에서 눌렀든 push를 눌렀든 뒤로가기는 사용자가 보고 있던 화면으로 돌아간다. 알림을
한 번 눌렀으면 history entry도 하나다.

앱이 종료된 상태에서 열린 창에는 돌아갈 화면 자체가 없으므로, 목적지가 앱 안에서 놓여 있던
자리를 밑에 깔고 그 위에 목적지를 얹는다. 그룹 게시물 알림이라면 `/` → `/groups` →
`/groups/:slug` 위에 게시물이 놓인다. 깔아 둔 화면은 실제로 그리지 않으므로 사용자가 실제로 뒤로
갔을 때 비로소 그 화면의 loader가 돈다.

게시물은 그룹 화면 위에 얹히는 오버레이지만, 그렇다고 뒤로가기 내역에 그룹을 끼워 넣지는 않는다.
가본 적 없는 화면이 뒤로가기에서 나오고 알림을 연달아 확인할 때마다 뒤로가기가 한 번씩 더 들기
때문이다. 게시물에서 그룹으로 가는 길은 상세 머리의 그룹 링크가 맡는다 — 뒤로가기에 숨겨 두는
것보다 보이는 편이 낫다.
