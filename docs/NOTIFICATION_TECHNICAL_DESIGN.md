...
멘션, 익명 활동 제한, 메시지, 사용자 지정 그룹 초대, 동아리, 노래방 참여자, 예약 리마인더,
...
되돌려 보내므로 사용자가 알림으로 들어온 화면에서 빠져나갈 수 없다. 목적지, 로그인, 알림함 어느
갈래로 빠지든 자기 entry를 replace로 갈아치운다. 로그인 화면도 같은 이유로 남기지 않는다.
