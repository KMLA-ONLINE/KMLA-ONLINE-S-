...
- 댓글 수와 게시물 반응은 `postId`별 TanStack Query engagement overlay를 카드·행·상세가 함께 구독한다. 생성·삭제 RPC가 돌려주는 `post_comment_count`는 절대값으로 그대로 얹고, 반응은 총계·내 반응만 먼저 계산한 뒤 서버 정본으로 덮는다. route 재검증이나 목록 원본·미디어 수화본 patch로 맞추지 마라 — 펼쳐 둔 답글 묶음과 이전 페이지를 유지하면서 늦은 수화본도 최신 표시를 되돌리지 못하게 한다. 명시적 서버 새로고침만 overlay를 먼저 비워 새 snapshot을 기준으로 삼는다.
