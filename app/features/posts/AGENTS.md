...
- 댓글 입력창은 `textarea`라 글자 일부만 달리 그릴 수 없다. 그래서 입력창이 드는 값은 원문이
  ...
  넘겨 보내고, `nextMentionOrdinal()`은 토큰이 없는 글에서 언제나 1을 준다. 이름이 같은
  대상이 여럿이면 나온 순서대로 짝을 짓는다. 동명이인이라고 그 하나만 토큰으로 보여 주면
  입력창에 원문이 다시 새어 나온다.
- 되돌릴 자리는 이름이 아니라 `MENTION_DISPLAY_MARK`(폭 없는 U+2060)로 찾는다. 버튼이 넣은
  자리에만 붙는다. 이름만 보고 짝을 지으면 골랐다가 지운 사람의 이름을 손으로 친 글이 멘션이
  되고, 탈퇴한 대상의 라벨이 같은 이름의 활성 대상에게 붙는다 — 둘 다 부른 적 없는 사람에게
  알림이 가는 길이다. 표시가 떨어져 나간 자리는 평문이 된다. 엉뚱한 사람을 부르는 것보다 안
  부르는 쪽이 낫다.
