...
- 되돌릴 자리는 이름이 아니라 버튼이 남긴 보이지 않는 표시(`MENTION_DISPLAY_MARKS`)로 찾고,
  그 표시가 같은 이름 안에서 몇 번째 사람인지까지 들고 다닌다. 이름만 보고 짝을 지으면 세
  가지가 한꺼번에 어긋난다: 골랐다가 지운 사람의 이름을 손으로 친 글이 멘션이 되고, 탈퇴한
  대상의 라벨이 같은 이름의 활성 대상에게 붙고, 동명이인 둘 중 하나를 지우면 남은 쪽이 먼저
  고른 사람을 부른다. 셋 다 부른 적 없는 사람에게 알림이 가는 길이다.
...
- 편집마다 `sanitizeMentionDisplay()`로 짝 잃은 표시를 걷는다. 남겨 두면 깨진 이름을 원래대로
