...
- `supabase/schemas/` is the source of truth for DDL. Edit the declarative file first, then use `npm run db:diff -- <name>` to generate a migration draft. Never make schema changes only in Studio, the SQL editor, or an imperative migration.
- Every generated migration is untrusted and must be read in full and corrected before `supabase migration up`, `db push`, or review handoff. In particular, check destructive statements, function drop/recreate and execute privileges, RLS, and column-level grants. See `supabase/README.md` for the mandatory checklist.
...
- Never edit a deployed migration. Use `npm run db:reset` to replay migrations plus `supabase/seed.sql`, and run a final `supabase db diff` to detect remaining tracked DDL drift.
- Preserve lexicographic schema dependencies: composite return types require their tables first, and triggers require their functions first. Cross-domain triggers belong no earlier than the trigger-function file. Run `supabase db diff` after any file split or renumbering to test the shadow build.
- Every new `public` table must explicitly revoke `MAINTAIN`, `REFERENCES`, `TRIGGER`, and `TRUNCATE` from `anon` and `authenticated`; dump output can omit these revokes because of seeded default privileges. Do not manage those defaults with dumped `ALTER DEFAULT PRIVILEGES` statements.
- Keep only project-owned `storage.objects` policies in `81-storage.sql`; never copy Supabase-managed Storage objects from a full schema dump. Keep required extension declarations in `01-foundation.sql` because dumps may omit them.
