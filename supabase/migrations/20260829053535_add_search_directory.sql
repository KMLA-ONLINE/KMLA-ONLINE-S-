...
GRANT ALL ON FUNCTION public.search_directory(text) TO authenticated;

ALTER TABLE public.profiles
  ADD COLUMN search_name text GENERATED ALWAYS AS (lower(regexp_replace(btrim(name), '[[:space:]]+'::text, ''::text, 'g'::text))) STORED;

CREATE INDEX profiles_search_name_trgm_idx ON public.profiles USING gin (search_name extensions.gin_trgm_ops)
  WHERE status = 'accepted'::public.profile_status AND deleted_at IS NULL;