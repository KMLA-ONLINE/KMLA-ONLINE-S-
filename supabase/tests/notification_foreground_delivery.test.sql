...
select is(
  ...
  1::bigint,
  'a normal notification is still claimed for the background device'
);
select is(
  (select endpoint from normal_claim),
  'https://push.example.test/background',
  'the foreground device alone is omitted from normal delivery'
);
reset role;
select is(
  ...
  'the foreground normal delivery is permanently suppressed'
);
...
select * from public.claim_notification_deliveries(10, 60);
select is(
  (select count(*) from expired_claim where title = '만료 후 일반 알림'),
  2::bigint,
  'normal delivery resumes on every device after heartbeat expiry'
);
select is(
  (select count(*) from expired_claim
   where importance = 'normal' and category = 'content'
